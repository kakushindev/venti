...
export const devs: string[] = JSON.parse(process.env.CONFIG_DEVS || "[]") as string[];
export const devGuilds = JSON.parse(process.env.CONFIG_DEV_GUILDS || "[]") as string[];
export const isDev = process.env.NODE_ENV === "development";
export const prefix = process.env.CONFIG_PREFIX;
export const deleteQueueTimeout = Number(process.env.CONFIG_DELETE_QUEUE_TIMEOUT) * 1_000;
export const lavalinkRest: { host?: string; auth?: string; } = JSON.parse(process.env.LAVALINK_REST || "{}") as { host?: string; auth?: string; };
...
if (typeof process.env.CONFIG_DELETE_QUEUE_TIMEOUT !== "string" || Number.isNaN(deleteQueueTimeout) || deleteQueueTimeout < 1) throw new Error("CONFIG_DELETE_QUEUE_TIMEOUT must be a number");
